...
public:
    ...
    DodgeGame(hittable_map* world, shared_ptr<sphere> player)
    ...
    void update(std::optional<SDL_Keycode> input, int frame) {
        ...
        // Movimento do player
        if (input) {
            if (*input == SDLK_a) player->center[0] -= 0.08;
            if (*input == SDLK_d) player->center[0] += 0.08;
        }
        // Limita área do player
        if (player->center[0] < -0.45) player->center[0] = -0.45;
        if (player->center[0] > 0.45) player->center[0] = 0.45;
        // Move obstáculos
